...
export function genAdAccounts(uid, n = 12) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // one decision drives both the code and its label, so the badge colour and
    // the text can never disagree (and the running-first sort looks right)
    const active = r() > 0.2;
    out.push({
      id: 'act_' + pad(1e14 + Math.floor(r() * 8e13)), account_id: pad(1e14 + Math.floor(r() * 8e13)),
      name: 'Ad Account ' + (i + 1), status: active ? 1 : 2,
      account_status_label: active ? '活跃' : '已停用', account_type: r() > 0.5 ? '后付费' : '预付费',
...
export function genPages(uid, n = 10) {
  ...
  for (let i = 0; i < n; i++) { const active = r() > 0.2; out.push({
    id: pad(1e14 + Math.floor(r() * 8e13)), name: 'Page ' + (i + 1), status: active ? 1 : 2,
    page_status_label: active ? '正常' : '停用', create_channel: r() > 0.5 ? '个人公共主页' : 'BM公共主页',
    created_time: '2025-0' + (1 + Math.floor(r() * 8)) + '-' + (10 + Math.floor(r() * 18)),
...
export function genPixels(uid, n = 9) {
  ...
  for (let i = 0; i < n; i++) { const active = r() > 0.3; out.push({
    ...
    role: ROLES[Math.floor(r() * ROLES.length)], is_active: active ? 1 : 0, active_label: active ? '活跃' : '不活跃',
